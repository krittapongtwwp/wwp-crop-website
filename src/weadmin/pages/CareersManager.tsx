...
export default function AdminCareers() {
  ...
  const filteredApplicants = applicants.filter((a) => {
    const searchLower = searchQuery.toLowerCase()
    if (searchQuery && !(a.name?.toLowerCase().includes(searchLower) || a.email?.toLowerCase().includes(searchLower)))
      return false
    return true
  })
        ...
        <form onSubmit={handleSave} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="wwp-card p-6 space-y-6">
              ...
              <div className="pt-4">
                <button type="submit" className="wwp-button-primary w-full flex items-center justify-center py-3">
    ...
    <div className="space-y-6">
      ...
      <div className="wwp-card p-4 flex flex-col sm:flex-row gap-4 justify-between items-center">
        <div className="flex space-x-1 w-full sm:w-auto overflow-x-auto pb-2 sm:pb-0 custom-scrollbar">
          {(activeTab === 'jobs'
            ? ['ทั้งหมด', 'เปิดรับสมัคร', 'ปิดรับสมัคร', 'ฉบับร่าง']
            : ['ทั้งหมด', 'ใหม่', 'ตรวจสอบแล้ว', 'สัมภาษณ์', 'รับเข้าทำงาน']
          ).map((tab) => (
            <button
              key={tab}
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors group">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          ...
                          <span
                            className={`w-1.5 h-1.5 rounded-full mr-1.5 ${item.status === 'open' ? 'bg-emerald-500' : 'bg-slate-400'}`}></span>
                      ...
                      <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors group">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-slate-900 dark:text-white font-medium">
                            {job?.title_th || job?.title_en || `Job ID: ${item.career_id}`}
                          </div>
                              ...
                              item.status === 'new'
                                ...
                                : item.status === 'reviewed'
                                  ...
                                  : item.status === 'interviewing'
                                    ? 'bg-purple-50 text-purple-700 dark:bg-purple-900/20 dark:text-purple-400 border border-purple-100 dark:border-purple-900/30'
                                    : item.status === 'hired'
                                      ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900/30'
                                      : 'bg-slate-100 text-slate-700 dark:bg-white/10 dark:text-slate-400 border border-slate-200 dark:border-white/10'
                            }`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-[#050A15] rounded-3xl shadow-2xl border border-slate-200 dark:border-white/10 w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
                  ...
                  <section className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 dark:bg-white/5 rounded-2xl p-6 border border-slate-100 dark:border-white/5">
                      ...
                      <div className="space-y-1">
                        ...
                        <a
                          href={`mailto:${viewApplicant.email}`}
                          className="text-base font-semibold text-blue-600 dark:text-blue-400 hover:underline flex items-center">
                      ...
                      <div className="space-y-1">
                        ...
                        <a
                          href={`tel:${viewApplicant.phone}`}
                          className="text-base font-semibold text-slate-900 dark:text-white hover:text-blue-600 transition-colors flex items-center">
                      ...
                      <div className="space-y-1">
                        ...
                        <span className="text-base font-semibold text-slate-900 dark:text-white">
                          {new Date(viewApplicant.created_at).toLocaleString('th-TH')}
                        </span>
                      </div>
                    </div>
                  </section>
                  ...
                  <section className="space-y-4">
                    ...
                    <div className="bg-slate-50 dark:bg-white/5 rounded-2xl p-6 border border-slate-100 dark:border-white/5 space-y-6">
                      <div className="space-y-1">
                        ...
                        <span className="text-base font-semibold text-slate-900 dark:text-white">
                          {jobs.find((j) => j.id === viewApplicant.career_id)?.title_th ||
                            `Job ID: ${viewApplicant.career_id}`}
                        </span>
                      </div>
                      <div className="space-y-1">
                        ...
                        {viewApplicant.portfolio_url ? (
                          <a
                            href={viewApplicant.portfolio_url}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-sm font-medium text-blue-600 dark:text-blue-400 hover:underline flex items-center break-all">
                  ...
                  <section className="space-y-4">
                    ...
                    <div className="wwp-card p-6 flex flex-col items-center text-center space-y-4">
                      ...
                      {viewApplicant.cv_url ? (
                        <a
                          href={getMediaUrl(viewApplicant.cv_url)}
                          target="_blank"
                          rel="noopener noreferrer"
            ...
            <div className="p-6 border-t border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 flex justify-end">
              <button onClick={() => setViewApplicant(null)} className="wwp-button-secondary px-8">
                ปิดหน้าต่าง
              </button>
            </div>
          </div>
