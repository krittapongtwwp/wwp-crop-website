...
export const generateAIContent = async (prompt: string, isJson: boolean = true): Promise<string> => {
  ...
  const res = await fetchApi('/ai/gen-content', {
    method: 'POST',
...
export const generateAIImage = async (prompt: string): Promise<string> => {
  ...
  //   }
  // }
  const response = await fetchApi('/ai/gen-image', {
    method: 'POST',
    body: JSON.stringify({ prompt })
  })

  if (!response.image_url) throw new Error('Failed to generate image from AI response')
